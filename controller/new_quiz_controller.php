<?php
    ...
    if(isset($_POST['new_quiz'])){
        ...
        $subj_id = $quiz_data->subj_id;
        $subj_name = $quiz_data->subj_name;
        $quiz_time = $quiz_data->quiz_time;
        $quiz_title = $quiz_data->quiz_title;
        $quiz_caption = $quiz_data->quiz_caption;
        $user_id = $quiz_data->user_id;
        $sql = "INSERT INTO quiz(quiz_title, subj_id, quiz_caption, quiz_num, quiz_time, user_id, status) VALUES (?,?,?,?,?,?,?)";
        $query = $dbcon->prepare($sql);
        $query->execute(array($quiz_title, $subj_id, $quiz_caption, 0, $quiz_time, $user_id, 1));
        if($query){
            echo "Swal.fire({icon:'success',html:'<span class=phetsarath>ບັນທຶກຂໍ້ມູນສໍາເລັດ!</span>',allowOutsideClick: false}).then((result) => {if (result.isConfirmed) {window.location.href='template?page=subject&sub_page=quiz&subj_id=".$subj_id."&subj_name=".$subj_name."'}});";
        }else{
            echo "Swal.fire({icon:'error',html:'<span class=phetsarath>ບັນທຶກຂໍ້ມູນບໍ່ສໍາເລັດ<br>ເກີດຂໍ້ຜິດພາດລະຫວ່າງການບັນທຶກ!</span>'})";
        }
    }
    if(isset($_POST['update_quiz'])){
        ...
        $quiz_caption = $quiz_data->quiz_caption;
        $user_id = $quiz_data->user_id;
        $sql = "UPDATE quiz SET quiz_title=?,subj_id=?,quiz_caption=?,quiz_time=?,user_id=? WHERE quiz_id = ?";
        $query = $dbcon->prepare($sql);
        $query->execute(array($quiz_title, $subj_id, $quiz_caption, $quiz_time, $user_id, $quiz_id));
        if($query){
            echo "Swal.fire({icon:'success',html:'<span class=phetsarath>ບັນທຶກຂໍ້ມູນສໍາເລັດ!</span>',allowOutsideClick: false}).then((result) => {if (result.isConfirmed) {window.location.href='template?page=subject&sub_page=quiz&subj_id=".$subj_id."&subj_name=".$subj_name."'}});";
        }else{
            echo "Swal.fire({icon:'error',html:'<span class=phetsarath>ບັນທຶກຂໍ້ມູນບໍ່ສໍາເລັດ<br>ເກີດຂໍ້ຜິດພາດລະຫວ່າງການບັນທຶກ!</span>'})";
