<?php
    ...
    function load_data(){
        global $dbcon;
        $sql = "SELECT id,class_des,user_id,(SELECT COUNT(*) FROM class_members mb WHERE mb.class_id = cr.id)'member',
        (SELECT COUNT(*) FROM class_subject cs WHERE cs.class_id = cr.id)subj_num FROM class_rooms cr WHERE status=1 ORDER BY id DESC";
