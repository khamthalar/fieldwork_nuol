<?php
    ...
    if(isset($_POST['load_question'])){
        ...
        $subj_id_param = "&subj_id=".$question_data->subj_id;
        $subj_name = $question_data->subj_name;
        $question_id_list = "";
        ...
        if($query){
            echo "Swal.fire({icon:'success',html:'<span class=phetsarath>ບັນທຶກຂໍ້ມູນສໍາເລັດ!</span>',allowOutsideClick: false}).then((result) => {if (result.isConfirmed) {window.location.href='template?page=subject&sub_page=quiz_preview".$quiz_id_param.$quiz_title_param.$subj_id_param."&subj_name=".$subj_name."'}});";
        }else{
    ...
    function load_question($quiz_id,$subj_id){
        ...
        WHERE q.status = 1 AND question_id NOT IN (SELECT question_id FROM quiz_question WHERE quiz_id = $quiz_id) AND q.subj_id = $subj_id ORDER BY question_id DESC";
