<?php
        ...
        function load_user_group_data(){
            require "config.php";
            $sql = "SELECT user_group_id,group_des,user_group_status,(SELECT COUNT(*) FROM tb_user u WHERE u.user_group_id = ug.user_group_id)'member' FROM tb_user_group ug WHERE user_group_status = 1";
            $query = $dbcon->prepare($sql);
            $query->execute();
            return $query;
        }
