...
$( document ).ready( function () {
    // if(parseInt(exam_data.selection_index)!=0){
    //     current_ques = parseInt(exam_data.selection_index);
    // }
    load_exam_data();
    // console.log(exam_data);
    var subj_name = exam_data.subj_name;
    $( "#subj_name" ).html( subj_name );
    // $('#active_ques').html((parseInt(exam_data.selection_index)+1));
    // $('#all_ques').html(exam_data.user_ans.length);
    load_button();
    load_question();
    load_ans();
} );
...
function load_ans() {
    ...
    if ( parseInt( question.question_type ) == 0 ) {
        ...
        question.ans_choice.forEach( _choice => {
            ...
            choice.className = "form-check";
            choice.innerHTML = `<input class="form-check-input choice-inp" type="checkbox" name="choice" id="choice` + _choice.choice_index + `" value="` + _choice.rang + `" ` + checked + `>
                                <label class="form-check-label phetsarath" for="choice`+ _choice.choice_index + `">` + _choice.choice + `</label>`;
            ans_box.appendChild( choice );
        } );
        answer.appendChild( ans_box );
...
function save_ans() {
    ...
    if ( question_type == 0 ) {
        ...
        if ( _ans.length != 0 ) {
            ...
            const old_ans_sirted = old_ans.slice().sort();
            if ( ans_data.length != old_ans.length && ans_data.slice().sort().every( function ( value, index ) {
                return value != old_ans_sirted[ index ];
            } ) ) {
                var correct_ans = question.correct_ans;
                const ans_sorted = correct_ans.slice().sort();
                if ( ans_data.length === correct_ans.length && ans_data.slice().sort().every( function ( value, index ) {
                    return value === ans_sorted[ index ];
                } ) ) {
                    question.point = question.full_point;
                } else {
                    question.point = 0;
