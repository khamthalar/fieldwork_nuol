DELIMITER $$
CREATE FUNCTION get_permission(id INT) RETURNS LONGTEXT
BEGIN
    DECLARE RETURN_VAL LONGTEXT;
    SET RETURN_VAL = (SELECT 
	CONCAT(
        '{"user_group_id":',ug.user_group_id,
        ',"user_group_des":"',ug.group_des,
        '","permission":', 
        	(SELECT CONCAT('[',GROUP_CONCAT('{', permission_data, '}' SEPARATOR ','),']') AS module FROM 
             	(SELECT CONCAT( 
                    ...
                    '","module":',module ) AS permission_data FROM 
                 ( 
                  		...
                  		(
                        	SELECT 
                     		...
                     		CONCAT ( 
                            	...
                            	'","allow":', (SELECT COUNT(*) FROM tb_group_permission gp WHERE gp.module_id = m.module_id AND gp.user_group_id = id) 
                        	) AS module_json
    ...
    ) AS permission_json_str          
FROM tb_user_group ug WHERE ug.user_group_id = id);
	RETURN (RETURN_VAL);
END$$
DELIMITER ;
