...
<div class="header-page">
  <div class="page-title">
    <h5 class="notosans"><a href="template?page=home" class="home-link">ໜ້າຫຼັກ</a> <i class="fas fa-chevron-right"></i><a href="template?page=user_group" class="home-link">ກຸ່ມຜູ້ໃຊ້ງານລະບົບ</a> <i class="fas fa-chevron-right"></i> ສິດການໃຊ້ງານລະບົບ</h5>
  </div>
</div>
      ...
      <div class="col-lg-12 grid-margin stretch-card">
        <div class="cus-card card">
          <div class="card-body">
            ...
            ?>
            <h4 class="card-title notosans">ສິດການໃຊ້ງານລະບົບ <?='('.$permission_data->user_group_des.')'?></h4>
              ...
              <div class="accordion" id="accordionPermission">
                ...
                <div class="accordion-item">
                  <h2 class="accordion-header" id="heading<?=$permission->module_group_id?>">
                    <button class="accordion-button collapsed notosans none-select none-outline" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?=$permission->module_group_id?>" aria-expanded="true" aria-controls="collapse<?=$permission->module_group_id?>">
                  ...
                  <div id="collapse<?=$permission->module_group_id?>" class="accordion-collapse collapse" aria-labelledby="heading<?=$permission->module_group_id?>" data-bs-parent="#accordionPermission">
                    <div class="accordion-body">
                      ...
                      <div class="action-btn">
                        <button type="button" class="btn btn-primary notosans">
                          <label class="notosans" style="margin: 0px;">
                            <input type="checkbox" hidden onchange="checkAll('check<?=$permission->module_group_id?>',this)"> ເລືອກທັງໝົດ
                          </label>
                        </button>
                        <button onclick="set_permission('<?=$permission->module_group_id?>')" type="button" class="btn btn-success notosans">ບັນທຶກ</button>
                      </div>
                    </div>
                  </div>
                </div>
...
<script>
  ...
  function set_permission(module_group_id){
    ...
    // if(permission_data.module.length!=0){
      ...
      Swal.fire( {
          html: '<h2 class="notosans">ກະລຸນາລໍຖ້າ !</h2><h4 class="notosans">ກໍາລັງໂຫຼດຂໍ້ມູນ</h4>',
          timer: 300,
        ...
        if(is_success){
          Swal.fire('<span class=notosans>ບັນທຶກສໍາເລັດ!</span>', '', 'success');
        }else{
          Swal.fire('<span class=notosans>ບັນທຶກຂໍ້ມູນບໍ່ສໍາເລັດ<br>ເກີດຂໍ້ຜິດພາດລະຫວ່າງການບັນທຶກ!</span>', '', 'error');
        }
      });
      Swal.stopTimer();
