    ...
    if(isset($_GET['subj_id'])){
      ...
      if(isset($_GET['quiz_id'])){
        ...
        $quiz_data = load_quiz($quiz_id)->fetchAll(PDO::FETCH_ASSOC)[0];
          ...
          echo "<script>window.location.href = 'template?page=subject&sub_page=quiz&subj_id=$subj_id&subj_name=$subj_name';</script>";
      }
    }else{
      ?>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group row">
                  <label class="col-form-label phetsarath f12">ວິຊາສອບເສັງ</label>
                  <div>
                    <select id="subject" class="inp form-control phetsarath f12" disabled>
                      <?php
                        $subjects = load_subject()->fetchAll(PDO::FETCH_ASSOC);
                        foreach ($subjects as $subject) {
                            $selected = "";
                            if($quiz_data['subj_id'] == $subject['subj_id'] ){
                                $selected = "selected";
                            }
                          echo "<option $selected value='" . $subject['subj_id'] . "'>" . $subject['subj_name'] . "</option>";
                        }
                      ?>
                    </select>
                  </div>
